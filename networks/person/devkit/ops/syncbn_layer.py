...
class SyncBNFunc(Function):
    ...
    def backward(ctx, grad_outdata):
        if grad_outdata.is_cuda:
            ...
            inDiff = scale_data / (var_bn.view(1, C, 1, 1) + ctx.eps).sqrt() * (
                        grad_outdata - 1 / (N * H * W * dist.get_world_size()) * (scaleDiff * x_hat + shiftDiff))
...
class SyncBatchNorm2d(Module):
    ...
    def forward(self, in_data):
        return SyncBNFunc.apply(
            in_data, self.weight, self.bias, self.running_mean, self.running_var, self.eps, self.momentum,
            self.training)
