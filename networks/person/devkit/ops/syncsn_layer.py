...
class SyncSNFunc(Function):
    @staticmethod
    def forward(ctx, in_data, scale_data, shift_data, mean_weight, var_weight, running_mean, running_var, eps, momentum,
                training):
        if in_data.is_cuda:
            ctx.eps = eps
            ...
            ctx.save_for_backward(in_data.data, scale_data.data, x_hat.data, mean.data, var.data, mean_in.data,
                                  var_in.data,
                                  mean_ln.data, var_ln.data, mean_bn.data, var_bn.data, mean_weight.data,
                                  var_weight.data)
        else:
    ...
    def backward(ctx, grad_outdata):
        if grad_outdata.is_cuda:
            ...
            term21 = var_weight[0] * 2 * (in_data.view(N, C, H, W) - mean_in.view(N, C, 1, 1)) / (H * W) * varDiff.view(
                N, C, 1, 1)
            term22 = var_weight[1] * 2 * (in_data.view(N, C, H, W) - mean_ln.view(N, 1, 1, 1)) / (
                        C * H * W) * torch.sum(varDiff, [1]).view(N, 1, 1, 1)
            term23_tmp = torch.sum(varDiff, [0]).view(1, C, 1, 1)
            dist.all_reduce(term23_tmp)
            term23 = var_weight[2] * 2 * (in_data.view(N, C, H, W) - mean_bn.view(1, C, 1, 1)) / (
                        N * H * W) * term23_tmp / dist.get_world_size()
            ...
            mean_weight_Diff[0] = mean_weight[0] * (
                        mw1_diff - mean_weight[0] * mw1_diff - mean_weight[1] * mw2_diff - mean_weight[2] * mw3_diff)
            mean_weight_Diff[1] = mean_weight[1] * (
                        mw2_diff - mean_weight[0] * mw1_diff - mean_weight[1] * mw2_diff - mean_weight[2] * mw3_diff)
            mean_weight_Diff[2] = mean_weight[2] * (
                        mw3_diff - mean_weight[0] * mw1_diff - mean_weight[1] * mw2_diff - mean_weight[2] * mw3_diff)
            var_weight_Diff[0] = var_weight[0] * (
                        vw1_diff - var_weight[0] * vw1_diff - var_weight[1] * vw2_diff - var_weight[2] * vw3_diff)
            var_weight_Diff[1] = var_weight[1] * (
                        vw2_diff - var_weight[0] * vw1_diff - var_weight[1] * vw2_diff - var_weight[2] * vw3_diff)
            var_weight_Diff[2] = var_weight[2] * (
                        vw3_diff - var_weight[0] * vw1_diff - var_weight[1] * vw2_diff - var_weight[2] * vw3_diff)
...
class SyncSwitchableNorm2d(Module):
    ...
    def forward(self, in_data):
        return SyncSNFunc.apply(
            in_data, self.weight, self.bias, self.mean_weight, self.var_weight, self.running_mean, self.running_var,
