...
# NOTE: THIS IS AN OLD VERSION OF THE ENSEMBLE SCRIPT AND NEEDS ADJUSTMENTS TO WORK PROPERLY AGAIN
###################################################################
...
from gcn_clustering.test import val_main, handle_intermediate_network
...
from misc import adjust_labeling, create_train_args, create_test_args, create_validation_output_file, \
    create_modified_gt_file, plot_knn_graph, plot_embedding_graph
...
intermediate_inputs = [[['spa'], ['spa'],  'labels_0.7_0.3', 'labels_0.7_0.3'],
                       [['reid'], ['spa'], 'labels_0.7_0.3', 'labels_0.7_0.3'],
                       [['person'], ['spa'], 'labels_0.7_0.3', 'labels_0.7_0.3']
                       ]
final_inputs = [['final'], ['spa'], 'labels_0.7_0.3', 'labels_0.7_0.3']
...
if training_type == 'combined':
    for detector_type in detector_types:
        for intermediate_input in intermediate_inputs:
            input_names = ''
            for inputs in intermediate_input[0]:
                input_names = input_names + inputs + '-'
            columns += ['train_' + detector_type + '_' + input_names + 'loss']
            columns += ['train_' + detector_type + '_' + input_names + 'avg_loss']
        ...
        for detector in detectors:
            for detector_type in detector_types:
                for intermediate_input in intermediate_inputs:
                    ...
                    for inputs in intermediate_input[0]:
                        input_names += inputs + '-'
                    columns += ['train_' + detector_type + '_' + detector[0] + input_names + 'loss']
                    columns += ['train_' + detector_type + '_' + detector[0] + input_names + '_avg_loss']
...
for iteration in range(iterations):
    ...
    # TRAINING
    for (train_detector, val_detector) in zip(train_seqs, val_seqs):
        print('\nTRAINING ' + str.upper(detector_types[det_num]) + ' DETECTORS')
        train_state_dicts = []
        if training_type == 'combined':
            print('\nTRAINING COMBINED GCN')
            train_feat_final = np.array([])
            train_intermediate_feat = np.array([])
            train_knn_graph_final = np.array([])
            for intermediate_input in intermediate_inputs:
                input_names = ''
                for inputs in intermediate_input:
                    input_names += inputs + '-'
                print('\nTRAINING COMBINED ' + str.capitalize(input_names) + 'GCN')
                ...
                for i, sequence in enumerate(train_detector):
                    ...
                    train_feat = np.array([])
                    bbox_size_idx = None
                    for input_feat in intermediate_input[0]:
                    ...
                    # load knn features
                    knn_feat = np.array([])
                    for input_feat in intermediate_input[1]:
                        if knn_feat.size == 0:
                            knn_feat = np.load(
                                os.path.join(train_directory, sequence[0], 'feat_' + input_feat + '.npy'))
                        else:
                    ...
                    train_labels = adjust_labeling(meta_train[intermediate_input[2]].to_numpy())
                ...
                intermediate_feat = handle_intermediate_network(state_dict=train_state_dict,
                                                                args=val_args,
                                                                input_channels=comb_train_feat.shape[1],
                                                                features=comb_train_feat,
                                                                knn_graph=comb_train_knn_graph,
                ...
                train_state_dicts.append(train_state_dict)

                if train_intermediate_feat.size == 0:
                    train_intermediate_feat = intermediate_feat
                else:
                    train_intermediate_feat = np.concatenate((train_intermediate_feat, intermediate_feat), axis=0)

            print('\nTRAINING FINAL GCN')
            train_labels_final = train_labels
            # check if feature basis is to be extended
            for inputs in final_inputs[0]:
                feat = np.array([])
                if inputs == 'final':
                    feat = train_intermediate_feat
                else:
                    if feat.size == 0:
                        feat = np.load(
                            os.path.join(train_directory, sequence[0], 'feat_' + inputs + '.npy'))
                    else:
                        feat = np.concatenate(
                            (feat, np.load(
                                os.path.join(train_directory, sequence[0], 'feat_' + inputs + '.npy'))), axis=1)
                    # how to handle spatial dataset
                    if inputs == 'spa':
                        if auto_correlate_feat_final:
                            feat = create_autocorrelation_dataset(feat)
                    # how to handle reid dataset
                    elif inputs == 'extra':
                        if handle_absolute_size == 'drop':
                            # drop absolute size column
                            feat = np.delete(feat, -1, axis=1)
                        elif handle_absolute_size == 'scale_by_dataset':
                            # rescale absolute size by dataset, i.e. between largest bbox being 1 and smallest 0
                            feat[:, -1] = (feat[:, -1] - feat[:, -1].min()) / \
                                          (feat[:, -1].max() - feat[:, -1].min())
                        elif handle_absolute_size == 'scale_by_batch':
                            # remember idx of absolute size column to later alter it during training time
                            bbox_size_idx = feat.shape[1] - 1
                        if handle_detector_confidence == 'drop':
                            # drop absolute size column
                            feat = np.delete(feat, -2, axis=1)
                if train_feat_final.size == 0:
                    train_feat_final = feat
                else:
                    train_feat_final = np.concatenate((train_feat_final, feat), axis=1)
            # create knn graph
            for inputs in final_inputs[1]:
                knn_feat = np.array([])
                if inputs == 'final':
                    knn_feat = train_intermediate_feat
                else:
                    if knn_feat.size == 0:
                        knn_feat = np.load(
                            os.path.join(train_directory, sequence[0], 'feat_' + inputs + '.npy'))
                    ...
                    # how to handle spatial dataset
                    if inputs == 'spa':
                        if auto_correlate_knn_feat_final:
                            knn_feat = create_autocorrelation_dataset(knn_feat)
                    # how to handle reid dataset
                    elif inputs == 'extra':
                        if handle_absolute_size == 'drop':
                            ...
                            # remember idx of absolute size column to later alter it during training time
                            bbox_size_idx = knn_feat.shape[1] - 1
                        if handle_detector_confidence == 'drop':
                            # drop absolute size column
                            knn_feat = np.delete(knn_feat, -2, axis=1)
                if train_knn_feat_final.size == 0:
                    train_knn_feat_final = knn_feat
                else:
                    train_knn_feat_final = np.concatenate((train_knn_feat_final, knn_feat), axis=1)

            # load filter dataset (concat feature datasets)
            filter_dataset_final = np.array([])
            for input_feat in knn_filter_dataset_final:
                if filter_dataset_final.size == 0:
                    filter_dataset_final = np.load(
                        os.path.join(train_directory, sequence[0], 'feat_' + input_feat + '.npy'))
                else:
                    filter_dataset_final = np.concatenate(
                        (filter_dataset_final, np.load(
                            os.path.join(train_directory, sequence[0], 'feat_' + input_feat + '.npy'))), axis=1)
                # how to handle spatial dataset
                if input_feat == 'spa':
                    if auto_correlate_filter:
                        filter_dataset_train = create_autocorrelation_dataset(filter_dataset_final)
                # how to handle reid dataset
                elif input_feat == 'extra':
                    if handle_absolute_size == 'drop':
                        # drop absolute size column
                        filter_dataset_train = np.delete(filter_dataset_final, -1, axis=1)
                    elif handle_absolute_size == 'scale_by_dataset' or handle_absolute_size == 'scale_by_batch':
                        # TODO: see above
                        # rescale absolute size by dataset, i.e. between largest bbox being 1 and smallest 0
                        filter_dataset_train[:, -1] = (filter_dataset_final[:, -1] - filter_dataset_train[:, -1]
                                                       .min()) / (filter_dataset_train[:, -1].max()
                                                                  - filter_dataset_train[:, -1].min())
                    if handle_detector_confidence == 'drop':
                        # drop absolute size column
            ...
            # reset learning rate
            train_args.lr = learning_rate_train
            train_state_dict_final, train_losses, train_avg_losses = train_main(args=train_args,
                                                                                input_channels=train_feat_final.shape[1],
                                                                                features=train_feat_final,
                                                                                knn_graph=train_knn_graph_final,
                                                                                labels=train_labels_final
                                                                                )

            eval_data.at[iteration, detector_types[detector_num] + 'final_loss'] = train_losses
            eval_data.at[
                iteration, detector_types[detector_num] + 'final_avg_loss'] = train_avg_losses

        print('\nVALIDATING ' + str.capitalize(detector_types[detector_num]) + ' DETECTORS')
        for i, sequence in enumerate(val_detector):
            val_labels_final = np.array([])
            val_intermediate_feat = np.array([])
            val_knn_graph_final = np.array([])
            # load meta
            meta_val = pd.read_csv(osp.join(directory, sequence[0], sequence[1]))
            print('Valdating ' + sequence[0] + '...')
            for j, intermediate_input in enumerate(intermediate_inputs):
                # name for intermediate detector (consists of all features used)
                input_names = ''
                for inputs in intermediate_input:
                    input_names += inputs + '-'
                ...
                for input_feat in intermediate_input[0]:
                    if val_feat.size == 0:
                        val_feat = np.load(os.path.join(directory, sequence[0], 'feat_' + input_feat + '.npy'))
                    else:
                        val_feat = np.concatenate((val_feat, np.load(
                            os.path.join(directory, sequence[0], 'feat_' + input_feat + '.npy'))), axis=1)

                # load knn features
                knn_feat = np.array([])
                for input_feat in intermediate_input[1]:
                    if knn_feat.size == 0:
                        knn_feat = np.load(os.path.join(directory, sequence[0], 'feat_' + input_feat + '.npy'))
                    else:
                        knn_feat = np.concatenate(
                            (knn_feat, np.load(os.path.join(directory, sequence[0], 'feat_' + input_feat + '.npy')))
                            , axis=1)
                ...
                # create splits and adjust labeling so that it goes from 0 to num_classes in split
                if split_type == 'train_valid':
                    val_labels = adjust_labeling(
                        meta_val[intermediate_input[2]].to_numpy()[meta_val['is_valid_' + split_type]])
                    val_feat = val_feat[meta_val['is_valid_' + split_type]]
                else:
                    val_labels = adjust_labeling(meta_val[intermediate_input[2]].to_numpy())

                if len(val_labels) > knn_val:
                    val_knn_graph = create_knn_graph_dataset(knn_feat, knn_val, knn_method)
                else:
                    val_knn_graph = create_knn_graph_dataset(knn_feat, len(val_labels) - 1, knn_method)

                # print shapes of files
                print('VALIDATION SHAPES ' + sequence[0] + '(' + str.capitalize(input_names) + ')')
                print(val_labels.shape)
                print(val_feat.shape)
                print(val_knn_graph.shape)

                if training_type == 'combined':
                    seq_array = np.full((len(val_feat), 1), 0)
                    val_feat = np.concatenate((seq_array, val_feat), axis=1)

                intermediate_feat = handle_intermediate_network(train_state_dicts[j], val_args,
                                                                val_feat.shape[1],
                                                                val_feat, val_knn_graph, val_labels)
                if val_intermediate_feat.size == 0:
                    val_intermediate_feat = intermediate_feat
                else:
                    val_intermediate_feat = np.concatenate((val_intermediate_feat, intermediate_feat), axis=0)


            # load ground truth files
            gt_val = np.genfromtxt(os.path.join(directory, sequence[0], 'gt/gt.txt'), dtype=float, delimiter=',')

            # load labels
            val_labels_final = val_labels

            # check if feature base is to be extended
            val_feat_final = np.array([])
            for inputs in final_inputs[0]:
                feat = np.array([])
                if inputs == 'final':
                    feat = val_intermediate_feat
                else:
                    feat = np.load(os.path.join(directory, sequence[0], 'feat_' + inputs + '.npy'))
                if val_feat_final.size == 0:
                    val_feat_final = feat
                else:
                    val_feat_final = np.concatenate((val_feat_final, feat), axis=1)

            # load knn features
            knn_feat_final = np.array([])
            for input_feat in final_inputs[1]:
                if knn_feat.size == 0:
                    knn_feat_final = np.load(os.path.join(directory, sequence[0], 'feat_' + input_feat + '.npy'))
                else:
                    knn_feat_final = np.concatenate(
                        (knn_feat_final, np.load(os.path.join(directory, sequence[0], 'feat_' + input_feat + '.npy')))
                        , axis=1)

            # create final knn graph
            if len(val_labels_final) > knn_val:
                val_knn_graph_final = create_knn_graph_dataset(knn_feat_final, knn_val, knn_method)
            else:
                val_knn_graph_final = create_knn_graph_dataset(knn_feat_final, len(val_labels_final) - 1, knn_method)

            # print shapes of files
            print('VALIDATION SHAPES ' + sequence[0] + '(FINAL)')
            print(val_labels_final.shape)
            print(val_feat_final.shape)
            print(val_knn_graph_final.shape)

            # if combined setting was used, then we need to append a 0 to the feature saying that all feature records
            # are from the same sequence
            if training_type == 'combined':
                seq_array = np.zeros((len(val_feat_final), 1))
                val_feat = np.concatenate((seq_array, val_feat_final), axis=1)

            val_pred, rm_filter, val_losses, val_avg_losses = val_main(train_state_dict_final, val_args,
                                                                       val_feat_final.shape[1], val_feat_final,
                                                                       val_knn_graph_final, val_labels_final)

            val_pred, val_pred_rm = create_validation_output_file(meta_val, val_pred, val_filter, rm_filter)
            val_mod_gt = create_modified_gt_file(gt_val, meta_val, val_filter)
            ...
            mkdir_if_missing(os.path.join('logs', log_date, log_timestamp, 'run_' + str(iteration + 1), 'full',
                                          sequence[0]))
            mkdir_if_missing(os.path.join('logs', log_date, log_timestamp, 'run_' + str(iteration + 1), 'removed',
                                          sequence[0]))
            ...
            if split_type == "train_valid_test" or split_type == "train_valid":
                np.savetxt(os.path.join('logs', log_date, log_timestamp, 'run_' + str(iteration + 1), 'full',
                                        sequence[0], 'gt.txt'), val_mod_gt, delimiter=',')
                np.savetxt(os.path.join('logs', log_date, log_timestamp, 'run_' + str(iteration + 1), 'removed',
                                        sequence[0], 'gt.txt'), val_mod_gt, delimiter=',')
